...
import com.example.zen.models.*
import com.example.zen.util.SerializationUtils
import com.example.zen.util.DateUtils
// removed unused coroutine/json imports

class ZenRepository private constructor(private val prefs: SharedPreferences, private val context: Context) {

    companion object {
        @Volatile
        private var INSTANCE: ZenRepository? = null

        fun getInstance(context: Context): ZenRepository {
            return INSTANCE ?: synchronized(this) {
                val prefs = context.getSharedPreferences("zen_prefs", Context.MODE_PRIVATE)
                val instance = ZenRepository(prefs, context.applicationContext)
                INSTANCE = instance
                instance
            }
        }
